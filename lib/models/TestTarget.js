var Util = require(__dirname + '/../Util.js'),
    ...
    store = new SimpleStore(__dirname + '/../../datastore/TestTargets');
...
var TestTarget = function() {
    this.__defineGetter__('iterate', function() { 
        return function(user, callback, fin) {
            store.each(function(td, put, del, stop) {
                callback({
                    name: td.n, type: td.t, status: td.s,
    ...
    this.__defineGetter__('match', function() { 
        var model = this;
        return function(user, regexp, callback, fin) {
            if(!regexp instanceof RegExp) fin(new Error('RegExp expected.'));
            else model.iterate(user, function(target, put, del, stop) {
                if(target.name.match(regexp))
                    callback(target, put, del, stop);
            }, fin);
        };
    });
